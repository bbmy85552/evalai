...
max_tokens = 20
model_name = "gemini-2.5-flash"  # 使用支持思考功能的模型
...
class GeminiStream:
    ...
    def stream(
        ...
        **extra,
        ...
        """
        流式调用，支持 reasoning_effort 和 thinking_config
    ...
    def chat_stream(self, prompt: str, max_tokens: Optional[int] = None, 
                   reasoning_effort: Optional[str] = None,
                   thinking_config: Optional[Dict[str, Any]] = None, **extra):
        """
        发送流式聊天请求
        ...
        if reasoning_effort or self.reasoning_effort:
            print(f"推理努力程度: {reasoning_effort if reasoning_effort else self.reasoning_effort}")
        if thinking_config:
            print(f"思考配置: {thinking_config}")
    ...
    def chat(self, prompt: str, max_tokens: Optional[int] = None,
             reasoning_effort: Optional[str] = None,
             thinking_config: Optional[Dict[str, Any]] = None, **extra):
        """
        非流式聊天请求
        ...
        try:
            ...
            raise GeminiChatError(f"聊天请求失败: {e}") from e
